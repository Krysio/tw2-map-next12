...
vec4 pixelToHexCoord(vec2 pixelPosition) {
    ...
    return hexCoords(rawCorods);
}

/*******************************/

void main() {
    vec4 hexCoord = pixelToHexCoord(pixelPosition);
    ...
    if (hexDistance > 0.8) {
        if (hexDistance > 1.0 - smoothWeight * 2.0) {
            vec4 secondHex;
            if (hexCoord.z > 0.0) {
                secondHex = pixelToHexCoord(vec2(
                    pixelPosition.x + 0.5,
                    pixelPosition.y
                ));
            } else {
            ...
            vec4 secondColor = getColor(1.0, secondHex);
            //secondColor = texture2D(iColor, vec2(0.0, 0.0));
            float colorParam = smoothstep(0.8, 1.0, hexDistance - 0.1 - smoothWeight); 
            color = vec4(
